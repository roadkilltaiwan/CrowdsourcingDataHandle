...
function get_attribute($tag, $attribute)
    ...
    $double_quote = return_between($cleaned_html, strtoupper($attribute)."=\"", "\"", EXCL); 
    $single_quote = return_between($cleaned_html, strtoupper($attribute)."='", "'", EXCL);
    if (strlen($double_quote)!=0) {
	    # Use return_between() to find the properly quoted value for the attribute
	    return $double_quote; 
		//return_between($cleaned_html, strtoupper($attribute)."=\"", "\"", EXCL);
    }
    else if (strlen($single_quote)!=0) {
	    return $single_quote;
    }
    else {
	return "";
